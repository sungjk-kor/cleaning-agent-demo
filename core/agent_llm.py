...
## 사용 모델: 반물리 5단계 소일링 모델 (IEA PVPS Task 13 / Coello-Boyle 계열)
1단계 PM 분리(PM_coarse = PM10 - PM2.5) → 2단계 일퇴적(Δm = 0.0864·cosθ·(v_f·PM2.5 +
v_c·PM_coarse)·F_site) → 3단계 강우세정(임계 R0=2.5mm, 최대 eta_max=0.8) → 4단계 누적/재비산 →
5단계 비선형 손실(SL = 1 - exp(-κ·m^γ), κ=0.0416). F_site는 지역특성 계수(일반=1.0, 산업·건조는 배수).
검증(서산 2025): 일반 3.4% / 산업 6.5% / 건조농업 8.0% / 극심 9.4% (IEA 세계평균 3~5%, 산업·건조 ~10% 부합).

{REPORT_KNOWLEDGE}

## 리포트 작성 지침
모든 도구 호출이 완료되면 분석 결과를 한국어 마크다운 리포트로 작성하세요. 다음을 포함하세요:
- **결과 요약**: 연평균 손실률, 발전량 손실, 세척 우선순위 Top 항목.
- **원인 설명(중요)**: 위 '소일링 도메인 지식'을 근거로, 왜 이 지역/기간에 이런 손실이
  나왔는지 물리적·기후적으로 설명하세요. 예) 무강수기간·봄철 고미세먼지·해안 염분·유효
  세정강우 부족 등을 실제 데이터와 연결. 관련 보고서 근거를 함께 언급(IEA T13-21:2022,
  2026 검토논문).
- **지역특성 해석**: 적용된 F_site와 각 지역요인(농업/산업/도로/해안/저틸트)이 소일링에
  기여하는 메커니즘을 설명.
- **세척 권고안**: 우선순위 날짜와 근거(누적먼지·무강수·고농도), 자연강우 세정 가능성 고려.
- **모델 한계**: 계수 현장보정 필요성, F_site 상대비교 성격을 반드시 명시.
...
class LLMAgentResult:
    ...
    lcoe: Any = None
    f_site_info: dict | None = None  # {f_site, breakdown, r6/r7 levels}
...
def run_llm_agent(
    ...
    def _exec_run_pollution_model(inp: dict) -> str:
        ...
        # 지역특성 → F_site 계수 (evaluate 단계 결과 재사용, 없으면 직접 계산)
        if state.get("f_site_info"):
            f_site_val = state["f_site_info"]["f_site"]
        else:
            f_site_val, _ = fsite_from_characteristics(state.get("regional_characteristics"))
        ...
        if f_site_val > 1.001:
            regional_note = (
                f"\n- 지역특성 F_site={f_site_val:.2f} "
                f"(PM/강수 기저 {pollution.annual_pm_loss_pct:.2f}% "
                f"+ 지역증가 {pollution.annual_regional_weight_ppt:.2f}%p "
                f"= {pollution.annual_pollution_loss_pct:.2f}%)"
            )
